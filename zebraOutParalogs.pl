...
open(OUT,">zebraOutParalogs.txt") ||die("zebraOutParalogs.txt does not open");
...
my $gene_members = $seq_member_adaptor->fetch_all_by_source_genome_db_id("ENSEMBLPEP",110);
my $count = 0;
my $paralogs = 0;
