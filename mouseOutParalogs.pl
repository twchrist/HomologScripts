...
open(OUT,">mouseOutParalogs.txt") ||die("mouseOutParalogs.txt does not open");
...
my $paralogs = 0;
 	 foreach my $this_member (@{$gene_members}) {
	    #now fetch in paralogues for first gene member
		    ...
		    my $homologies = $homology_adaptor->fetch_all_out_paralogues_from_Member_NCBITaxon($this_member, $zebra_genome_db->taxon);
